...
const Posts = ({ userData, searchedUser, isMyProfile }) => {
  return (
    <div className="bg-bgprimary flex flex-col items-center rounded-2xl shadow-hard">
      ...
      {!userData?.length ? (
        <div className="flex justify-center items-center min-h-[400px] gap-2 px-5">
          <p className="text-textcolor font-route text-title3">
            {isMyProfile ? "You have" : `${searchedUser?.username} has`} not
            created any posts.
          </p>
        </div>
      ) : (
